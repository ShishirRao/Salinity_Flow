...
ggdraw() +
  ...
  draw_plot(p_inset1, x = 0.55, y = 0.70, width = 0.4, height = 0.24)+ # Position and size of inset
  draw_plot(p_inset2, x = 0.55, y = 0.26, width = 0.4, height = 0.24) # Position and size of inset
...
Gang_fig = left_join(Date_time,Gang_depth %>% select(Date_time,depth))
Gang_fig = left_join(Gang_fig,Sal_Gang %>% select(Date_time,Salinity,month))
...
p_main <- ggplot(Gang_fig[Gang_fig$Date_time <= ymd_hms("2023-11-14 17:00:00"),], aes(x = Date_time, y = value)) + geom_line() + 
  facet_grid(parameter ~ ., scales = "free_y") + theme(legend.position = "none")+
  scale_x_datetime(date_labels = "%b\n%Y",date_breaks = "30 days")+theme_bw() +
  theme(axis.text=element_text(size=18),
        axis.title=element_text(size=20,face="bold"),
...
# Create inset plot
p_inset1 <- ggplot(Gang_fig[Gang_fig$parameter == "depth" & Gang_fig$month == 6,], aes(x = Date_time, y = value)) + geom_line() + 
  scale_x_datetime(date_labels = "%d-%b",date_breaks = "5 days")+theme_bw() +
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=20,face="bold"),
        plot.title = element_text(size = 20, face = "bold"))+
...
# Create inset plot
p_inset2 <- ggplot(Gang_fig[Gang_fig$parameter == "Salinity" & Gang_fig$month == 4,], aes(x = Date_time, y = value)) + geom_line() + 
  scale_x_datetime(date_labels = "%d-%b",date_breaks = "5 days")+theme_bw() +
  theme(axis.text=element_text(size=18),
        axis.title=element_text(size=20,face="bold"),
        plot.title = element_text(size = 20, face = "bold"))+
...
ggdraw() +
  draw_plot(p_main, x = 0, y = 0, width = 1, height = 1) +
  draw_plot(p_inset1, x = 0.55, y = 0.75, width = 0.4, height = 0.2)+ # Position and size of inset
  draw_plot(p_inset2, x = 0.55, y = 0.25, width = 0.4, height = 0.2) # Position and size of inset
...
# Create main plot
p_main <- ggplot(Shar_fig[Shar_fig$Date_time <= ymd_hms("2023-11-14 17:00:00"),], aes(x = Date_time, y = value)) + geom_line() + 
  facet_grid(parameter ~ ., scales = "free_y") + theme(legend.position = "none")+
  scale_x_datetime(date_labels = "%b\n%Y",date_breaks = "30 days")+theme_bw() +
  theme(axis.text=element_text(size=18),
        axis.title=element_text(size=20,face="bold"),
...
ggsave("E:/Shishir/FieldData/Results/Shar_Sal_Depth_v3.jpg",  width = 6, height = 3.5,scale = 3)
...
Kali_fig$value[Kali_fig$parameter == "depth" ]  = Kali_fig$value[Kali_fig$parameter == "depth" ] + 10.0 
